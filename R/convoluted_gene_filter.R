...
test_data <- function(){
  tf <- matrix(rep(0, 48), ncol = 12)
  colnames(tf) <- c(paste0('a', 1:3), paste0('b', 1:4), paste0('c', 1:5))
  groups <- data.frame(g=substr(colnames(tf), 1,1), row.names = colnames(tf))
  row.names(tf) <- paste0('gene', 1:4)
  #tf[1:2,1:3] <- 5
  tf[1,] <- 5
  tf[2,4:7] <- 9
  tf[3,4:5] <- 8
